...
# $Id: p11advfigs.py 678 2012-09-11 11:32:57Z patricio $
...
def main(args):
    ...
    for i in np.array((0,1,2,3)):
	    weight[i] = 1/(fit[i].std[fit[i].i.midpt])**2
    ...
    for i in range(nfit):
        ...
        for j in range(nbins[i]):
          ...
          enduc                 = int(1.*(j+1)*fit[i].nobjuc/nbins[i])
          fit[i].binphaseuc[j]  = np.mean(fit[i].phaseuc[startuc:enduc])
          fit[i].binfluxuc[j]   = sum(fit[i].fluxuc[startuc:enduc]/fit[i].sigmauc[startuc:enduc]**2)/ sum(1/fit[i].sigmauc[startuc:enduc]**2)
...
def combineEvents(fit, fitlist, numbins, eclipse=True):
  ...
  for i in fitlist:
    if eclipse == True:
      # Secondary eclipses
      time   = np.concatenate((time,    fit[i].phase))
      timeuc = np.concatenate((timeuc,  fit[i].phaseuc))
    else:
      # Primary transits
      time   = np.concatenate((time,    fit[i].ephdat))
  ...
  for j in range(numbins):
    startuc          = int(1.0* j   *nobjuc/numbins)
    enduc            = int(1.0*(j+1)*nobjuc/numbins)
