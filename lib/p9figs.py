...
def figs(event, fitnum, fignum, titles=False):
    #Define class
    ...
    # Change thinning to int for updated python
    ...
    if titles:
        plt.suptitle('Binned ' + event.params.planetname +\
                     ' Data With Eclipse Models', size=18)
    ...
    plt.errorbar(abscissa,fit.normbinflux,fit.normbinstd,fmt='ko',ms=4,linewidth=1, label='Binned Data')
        ...
        for i in fit.nonfixedpars[1:numfp]:
            ...
            for j in fit.nonfixedpars[0:numfp-1]:
                ...
                if i > j:
                    ...
                    plt.plot(fit.allparams[j,0::thinning],fit.allparams[i,0::thinning],'b,') #,ms=2)
                k += 1
                n +=1
            m +=1
        ...
        for i in fit.nonfixedpars[num1:numfp]:
            ...
            for j in fit.nonfixedpars[0:num1-1]:
                ...
                if i == fit.nonfixedpars[numfp-1]:
                    ...
                    a = plt.xticks(visible=False)
                plt.plot(fit.allparams[j,0::thinning],fit.allparams[i,0::thinning],'b,') #,ms=1)
                k += 1
                n += 1
            m += 1
        plt.subplots_adjust(left=0.15,right=0.95,bottom=0.15,top=0.95,hspace=0.15,wspace=0.15)
    ...
    #a = plt.plot(fit.binsz, fit.rms, color='black', lw=1.5, label='RMS')
    #a = plt.plot(fit.binsz, fit.stderr, color='red', ls='-', lw=2,
    #             label='Std. Err.')
    #a = plt.yscale('log')
    #a = plt.xscale('log')
    ...
    a = plt.xlim(fit.binsz[0]/2, fit.binsz[-1]*2)
    ...
    if titles:
        a = plt.suptitle(event.params.planetname + ' Correlated Noise',
                         size=18)
    ...
    a = plt.subplot(1,2,1)
    a = plt.errorbar(yround+fit.binyy, fit.binyflux, fit.binyflstd, fmt='ro',
                     label='Binned Flux', zorder=1)
    a = plt.plot(yround+fit.binyy, fit.binybestip, 'k-', lw=2,
                 label='BLISS Map', zorder=2)
    a = plt.xlabel('Pixel Postion in y', size=14)
    a = plt.ylabel('Normalized Flux', size=14)
    a = plt.xticks(rotation=90)
    a = plt.legend(loc='best')
    a = plt.subplot(1,2,2)
    a = plt.errorbar(xround+fit.binxx, fit.binxflux, fit.binxflstd, fmt='bo',
                     label='Binned Flux', zorder=1)
    a = plt.plot(xround+fit.binxx, fit.binxbestip, 'k-', lw=2,
                 label='BLISS Map', zorder=2)
    ...
    for i in fit.ifreepars:
        ...
        a  = plt.hist(fit.allparams[i,0::thinning], 20, label=str(fit.meanp[i]))
        j += 1
    ...
    for files in os.listdir('.'):
        if files.endswith('.ps'):
            os.chmod(files, 664)   #0664 must be in octal
