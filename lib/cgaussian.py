...
def gaussian(x, width=1.0, center=0.0, height=None, bgpars=[0.0, 0.0, 0.0]):
  """
    ...
    2016-12-11 0.6  jh@physics.ucf.edu  Fixed array comparisons with None.
  """
...
def old_gaussianguess(y, x=None):
  """
    ...
    2016-12-11 0.5 jh@physics.ucf.edu    Fixed array comparisons with None.
  """
...
def fitgaussian(y, x=None, bgpars=None, fitbg=0, guess=None,
  ...
  """
    ...
    >>> # x and y data to fit
    >>> nx = (hx - lx) / dx + 1
    >>> x  = np.indices((int(nx), int(nx))) * dx + lx
    ...
    2011-05-03  patricio Added mask, weights, and background-fitting options.
                         pcubillos@fulbrightmail.org
    2016-12-11 jh        Fixed array comparisons with None.
    2017-02-02 rchallen  Fixed raises to allow use with python 3
  """
...
def residuals(params, x, data, mask, weights, bgpars, fitbg):
  ...
  # Produce the model:
  model = cg.make_gauss(n, m, x0, y0, sigmax, sigmay, height, bgparams[2])
  # Calculate residuals:
...
def fitgaussians(y, x=None, guess=None, sigma=1.0):
  ...
  iscov = 0 if type(cov)==type(None) else 1
...
def rotfitgaussian(y, x=None, bgpars=None, fitbg=0, guess=None,
                   mask=None, weights=None, maskg=False, yxguess=None):
  ...
  # The fit:
  p, cov, info, mesg, success = so.leastsq(rotresiduals, params, args,
                                           full_output=True)
...
def rotresiduals(params, x, data, mask, weights, bgpars, fitbg):
  '''
  Calculates the residuals between data and a rotated gaussian model.
  Used in rotfitgaussian. See residuals() for reference.

  Revisions
  ---------
  2018-08-29 rchallen Initial version based on residuals()
  ...
  width  = params[0        :  data_dims]
  center = params[data_dims:2*data_dims]
  if params_len - 2*data_dims == 2:
    height = params[2*data_dims  ]
    theta  = params[2*data_dims+1]
  else:
    # Parameters are the wrong length for 2D rotated Gaussian. Resort to
    # defaults. 
    height = 1.0 / ((2 * np.pi)**0.5 * sigmax * sigmay)
    theta  = 0
    print("Warning: parameter length suspicious!")

  n, m = np.shape(data)
  y0, x0 = center
  sigmay, sigmax = width

  # Produce the model (move to C eventually)
  y, x = np.indices((n, m))
