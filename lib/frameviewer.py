...
def frameviewer(event, zoom=True, expand=14, scale=100.0):
  """
    ...
    event: An Event instance
       An event after p3center. If an event after p4photom is
       is supplied, it  will display the star and sky apertures as well.
    zoom: boolean
       If True, display a zoom area around the star. Necessary for 
       large arrays.
    expand:
    scale: Float
       Clip color scale to contain a 'scale' percentile of the flux
       distriburion around the median.
    ...
    2014-04-21  patricio    Added scale argument.
  """
  ...
  def loadimage(index, posit):
    ...
    # Clip scale to selected percentile:
    perc = (100.0-scale)/2.0
    vm, vM = np.percentile(event.data[index,:,:,posit], [perc, 100-perc])
    im = np.clip(im, vm, vM)
...
#    print(' ' + str(event.fp.x[index,posit])+' '+str(event.fp.y[index,posit]))
    ...
    if aper:
      pg.draw.circle(image, red,  (int(x*expand), int(y*expand)), int(photap*expand), 1)
      pg.draw.circle(image, blue, (int(x*expand), int(y*expand)), int(skyin *expand), 1)
      pg.draw.circle(image, blue, (int(x*expand), int(y*expand)), int(skyout*expand), 1)
