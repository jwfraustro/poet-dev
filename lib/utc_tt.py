...
#rundir = '/home/esp01/ancil/leapseconds/'

def leapdates(leapdir):
	...
	'''
	try:
		files = os.listdir(leapdir)
		recent = np.sort(files)[-1]
		nist = open(leapdir+recent, 'r')
		doc = nist.read()
		nist.close()
		...
		if time.time()+ ntpepoch > expiration:
			...
			nist = urllib.request.urlopen('https://www.ietf.org/timezones/data/leap-seconds.list')
			doc = str(nist.read(), 'utf-8')
			nist.close()
			newexp = doc.split('#@')[1].split('\n')[0][1:]
			newfile = open(leapdir+"leap-seconds."+newexp, 'w')
			newfile.write(doc)
			newfile.close()
			table = doc.split('#@')[1].split('\n#\n')[1].split('\n')
...
def bjdcorr(date, ra, dec, hordir, location="s"):
...
#	horizonsdir = "/home/esp01/ancil/horizons/"
	if location == "s":
		...
		tabfile = "all_sun.vec"
	bjd = suntimecorr(hms_rad(*ra), dms_rad(*dec), date, hordir+tabfile)
	return bjd

def hms_rad(hour, minute, second):
    #Converts right ascension from hh:mm:ss to radians
    return (hour + minute/60. + second/3600.)*np.pi/12

def dms_rad(degree, minute, second):
