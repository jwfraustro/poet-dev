#define NPY_NO_DEPRECATED_API NPY_1_14_API_VERSION

#include<Python.h>
#include<numpy/arrayobject.h>
#include<math.h>

#define IND(a,i) *((double *)(PyArray_DATA(a)+i*PyArray_STRIDES(a)[0]))

static PyObject *felramp(PyObject *self, PyObject *args, PyObject *keywds);

static PyObject *felramp(PyObject *self, PyObject *args, PyObject *keywds)
{
  PyObject *etc;
  PyArrayObject *t,*y,*rampparams;
  double goal,m,t0,a,t1;
  int i;
  npy_intp dims[1];

  //etc = PyList_New(0);

  static char *kwlist[] = {"rampparams","t","etc",NULL};

  if(!PyArg_ParseTupleAndKeywords(args,keywds,"OO|O"		\
				  ,kwlist,&rampparams,&t,&etc))
    {
      return NULL;
    }

  goal = IND(rampparams,0);
  m    = IND(rampparams,1);
  t0   = IND(rampparams,2);
  a    = IND(rampparams,3);
  t1   = IND(rampparams,4);

  dims[0] = PyArray_DIMS(t)[0];

  y = (PyArrayObject *) PyArray_SimpleNew(1,dims,NPY_DOUBLE);

  for(i=0;i<dims[0];i++)
    {
      IND(y,i) = goal*(1+exp(-1*m*(IND(t,i)-t0)))+a*(IND(t,i)-t1);
    }

  return PyArray_Return(y);
}


static PyMethodDef felramp_methods[] = {
  {"felramp",(PyCFunction)felramp,METH_VARARGS|METH_KEYWORDS},{NULL}};

static char felramp_module_doc[]="\
This is a test, I wonder were this will show up.\
";

static struct PyModuleDef felramp_module=
{
    PyModuleDef_HEAD_INIT,
    "felramp",             // name of module 
    felramp_module_doc, // module documentation, may be NULL 
    -1,                     // size of per-interpreter state of the module, or -1 if the module keeps state in global variables. 
    felramp_methods
};

PyMODINIT_FUNC PyInit_felramp(void)
{
  import_array();
  return PyModule_Create(&felramp_module);
}

/*
void initfelramp(void)
{
  Py_InitModule("felramp",felramp_methods);
  import_array();
}
*/
