...
import smoothing
from aorflux import aorflux

def setupmodel(model, ind=rd.Pcf({'size':0}), npldpars=9):
   """
  ...
  -------
   ...
   for i in range(len(model)):
        if   model[i] == 'mandelecl':
                ...
                ind.grp    = ind.size + 3
                ind.gp     = ind.size + 4
                ind.gdepth = ind.size + 5
                ind.ga     = ind.size + 6
                ind.gflux  = ind.size + 7
                ind.size  += 8
                #DEFINE NAMES
                parname.insert(ind.gmidpt,'Eclipse Phase')
                parname.insert(ind.gb,    'Impact Parameter')
                parname.insert(ind.grs,   'Stellar Radius')
                parname.insert(ind.grp,   'Planet Radius')
                parname.insert(ind.gp,    'Period')
                parname.insert(ind.gdepth,'Maximum Eclipse Depth')
                parname.insert(ind.ga,    'Semi-major Axis')
                parname.insert(ind.gflux,r'System Flux ($\mu Jy$)')
                #DEFINE ECLIPSE MODEL
                myfuncs.append(mc.mandelgraz)
                saveext.append('mg')
                functype.append('ecl/tr')
                ...
                #DEFINE NAMES
                parname.insert(ind.trmidpt,  'Transit Midpoint')
                parname.insert(ind.trdepth,  'Transit Depth')
                parname.insert(ind.trin,     'Ingress Time')
                parname.insert(ind.treg,     'Egress Time')
                parname.insert(ind.trflux,  r'System Flux ($\mu Jy$)')
                #DEFINE ECLIPSE MODEL
                myfuncs.append(mc.asech)
                saveext.append('as')
                functype.append('ecl/tr')
        elif model[i] == 'ortho':
                #DEFINE INDICES
        ...
        # FINDME ccampo added 2010-08-20
                ...
                saveext.append('sc')
                functype.append('sinusoidal')
        elif model[i] == 'sincosquad':
                #DEFINE INDICES
                ind.scqa     = ind.size
                ind.scqp1    = ind.size + 1
                ind.scqt1    = ind.size + 2
                ind.scqb     = ind.size + 3
                ind.scqp2    = ind.size + 4
                ind.scqt2    = ind.size + 5
                ind.scqc     = ind.size + 6
                ind.scqr2    = ind.size + 7
                ind.scqr1    = ind.size + 8
                ind.scqt3    = ind.size + 9
                ind.size   += 10
                #DEFINE NAMES
                parname.insert(ind.scqa,   'Sine, Amplitudet')
                parname.insert(ind.scqp1,  'Sine, Period')
                parname.insert(ind.scqt1,  'Sine, Phase Offse')
                parname.insert(ind.scqb,   'Cosine, Amplitudet')
                parname.insert(ind.scqp2,  'Cosine, Period')
                parname.insert(ind.scqt2,  'Cosine, Phase Offse')
                parname.insert(ind.scqc,   'Sin/Cos, Constant Term')
                parname.insert(ind.scqr2,  'Quadratic Coefficient')
                parname.insert(ind.scqr1,  'Linear Coefficient')
                parname.insert(ind.scqt3,  'Polynomial Phase Offset')
                #DEFINE RAMP MODEL
                myfuncs.append(mc.sincosquad)
                saveext.append('scq')
                functype.append('ramp')
        elif model[i] == 'asymcos':
                #DEFINE INDICES
                ind.acc1     = ind.size
                ind.acc2     = ind.size + 1
                ind.acc3     = ind.size + 2
                ...
                ind.size   += 6
                #DEFINE NAMES
                parname.insert(ind.acc1,  'Cos 1 Amp')
                parname.insert(ind.acc2,  'Cos 1 Pos')
                parname.insert(ind.acc3,  'Cos 2 Amp')
                parname.insert(ind.acc4,  'Cos 2 Pos')
                parname.insert(ind.acc5,  'Vertical Offset')
                parname.insert(ind.acp,   'Period')
                #DEFINE RAMP MODEL
                myfuncs.append(mc.asymcos)
                saveext.append('ac')
                functype.append('sinusoidal')
        elif model[i] == 'quadip':
                #DEFINE INDICES
                ...
                ind.qsy     = ind.size + 3
                ind.qsx     = ind.size + 4
                ind.qsipc   = ind.size + 5
                ind.size  += 6
                #DEFINE NAMES
                parname.insert(ind.qsy2,    'Intra-pixel, Quadratic Term in ysig')
                parname.insert(ind.qsx2,    'Intra-pixel, Quadratic Term in xsig')
                parname.insert(ind.qsxy,    'Intra-pixel, Cross Term')
                parname.insert(ind.qsy,     'Intra-pixel, Linear Term in ysig')
                parname.insert(ind.qsx,     'Intra-pixel, Linear Term in xsig')
                parname.insert(ind.qsipc,   'Intra-pixel, Constant Term')
                #DEFINE INTRA-PIXEL MODEL
                myfuncs.append(mc.quadsigip)
                saveext.append('qsip')
                functype.append('ippoly')
                ...
                #DEFINE INDICES
                for j in range(npldpars):
                   setattr(ind, 'p'+str(j+1), ind.size+j)
                ind.size += j+1
                #DEFINE NAMES
                for j in range(npldpars):
                   parname.insert(getattr(ind, 'p'+str(j+1)), 'P'+str(j+1))
                #DEFINE INTRA-PIXEL MODEL
                myfuncs.append(mc.pld)
                saveext.append('pld'+str(j+1))
                functype.append('pld')                
                ...
                ind.size += 8
                # DEFINE NAMES
                parname.insert(ind.a0, 'aor 0')
                parname.insert(ind.a1, 'aor 1')
                parname.insert(ind.a2, 'aor 2')
                parname.insert(ind.a3, 'aor 3')
                parname.insert(ind.a4, 'aor 4')
                parname.insert(ind.a5, 'aor 5')
                parname.insert(ind.a6, 'aor 6')
                parname.insert(ind.a7, 'aor 7')
                # DEFINE INTRA-PIXEL MODEL
                myfuncs.append(aorflux)
                saveext.append('af')
