...
def init_models(npldpars=9):
    models = [#ECLIPSES / TRANSITS
            ...
            ["mandelgraz",   "gmidpt    \tgb        \tgrs       \tgrp       \tgperiod   \tgdepth    \tga        \tgflux\n", np.zeros((4, 8))],
            ["asech",        "trmidpt   \ttrdepth   \ttrin      \ttreg      \ttrflux\n", np.zeros((4,5))],
            #RAMPS
            ...
            ["sincosquad",   "a\tp1\tt1\tb\tp2\tt2\tc\tr1\tr2\tt3\n", np.zeros((4, 10))],
            ["asymcos",      "c1\tc2\tc3\tc4\tc5\tP\n", np.zeros((4,6))],
            #INTRAPIXEL / INTERPOLATION / PLD
            ["linip", "y0\tx0\ty1\tx1\ty2\tx2\ty3\tx3\ty4\tx4\ty5\tx5\ty6\tx6\ty7\tx7\ty8\tx8\n", np.zeros((4, 18))],
            ["quadip", "a\tb\tc\td\te\tf\n", np.zeros((4, 6))],
            ["quadsigip", "a\tb\tc\td\te\tf\n", np.zeros((4, 6))],
            ["quadip4", "a\tb\tc\td\te\tf\n", np.zeros((4, 24))],
            ...
            ["pld", "\t".join([("p"+str(i)) for i in range(npldpars+1)])+"\n", np.zeros((4, npldpars))], # Allows for variable number of pld pars
            #OTHERS
            ["ortho",        "none\n", np.zeros((4, 1))],
...
def read(filename, selected_models, event, npldpars=9):

    if os.path.isfile(filename) == False:
        raise Exception("initvals file: " + filename + " not found. (Check " +
                        "params.pcf and make sure modelfile is of the form " +
                        "<eventname>-initvals.txt)")
    models = read_parameters(filename, npldpars)
...
def read_parameters(file_name, npldparams=9):
    models = init_models(npldparams)
    f = open(file_name, "r")
    #global comment
    comment = ""
