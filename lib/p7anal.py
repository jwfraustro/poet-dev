...
"""
 MODIFICATION HISTORY:
    ...
    Updated for new python      rchallen 2017-02-13
    Updated for Python 3        zacchaeus2018-01-06
"""
...
def stdanal(event, fit, plotnum, printout, isloadallknots=True):
    ...
    if fit.isipmapping and isloadallknots:
        ...
        usedknotsflat = readnpy(fit.allknotfile, numit).T
        ...
        plt.clf()
        plt.hist(usedknotsflat[0,::thinning]/300000.+1, 20)
        plt.xlabel('Knot 0')
    ...
    if fit.isipmapping == True and event.params.isfixipmap == False and isloadallknots:
        ...
        if hasattr(fit.i, 'depth') and fit.i.depth in fit.nonfixedpars:
            ...
            # fit.binfluxmask == 1 at as many oints as 
        ...
        #PLOT MIDPOINT
        if False: #hasattr(fit.i, 'midpt') and fit.i.midpt in fit.nonfixedpars: #ZINDME
    ...
    for i in range(len(fit.nonfixedpars)):
        fit.twosidederr[fit.nonfixedpars[i],:] = hiloerr.hiloerr( \
            allparams[fit.nonfixedpars[i],0::10], fit.medianp[fit.nonfixedpars[i]]) 
    ...
    if event.photchan == 5:
        event.exptime = 6.29
        event.gain    = 1 # just to make the code run
        print('WARNING: Assuming gain = 1. Do not trust photon-limited S/N.')
    ...
    fit.sn     = fit.systemflux / np.sqrt(np.mean(fit.sigma**2))
    ...
    if hasattr(fit.i, 'rprs'):
        ...
        fit.incerr  = hiloerr.hiloerr(incall[::thinning], np.arccos(cosimed)*180./np.pi)
        #Calculate transit depth and error
        fit.trdepth    = rprs*rprs
        trdepthall     = rprsall*rprsall
        fit.trdeptherr = hiloerr.hiloerr(trdepthall[::thinning], rprsmed*rprsmed)
        #Calculate impact parameter and errors
        fit.trb = ars*cosi
        medtrb  = arsmed*cosimed
        trball  = arsall*cosiall
        fit.trberr = hiloerr.hiloerr(trball[::thinning], medtrb)
        ...
        fit.trt14err = hiloerr.hiloerr(trt14all[::thinning], medtrt14)
        fit.trt12err = hiloerr.hiloerr(trt12all[::thinning], medtrt12)
        ...
        fit.rhostarerr = hiloerr.hiloerr(rhostarall[::thinning], medrhostar)
    ...
    if hasattr(fit.i, 'rprs2'):
        ...
        incall      = np.arccos(cosiall)*180./np.pi
        fit.incerr2 = hiloerr.hiloerr(incall[::thinning], np.arccos(cosimed)*180./np.pi)
        #Calculate transit depth and errors
        fit.trdepth2    = rprs*rprs
        trdepthall      = rprsall*rprsall
        fit.trdeptherr2 = hiloerr.hiloerr(trdepthall[::thinning], rprsmed*rprsmed)
        #Calculate impact parameter and errors
        fit.trb2= ars*cosi
        medtrb  = arsmed*cosimed
        trball  = arsall*cosiall
        ...
        trt23all = p/pi*np.arcsin(1/arsall*np.sqrt(((1-rprsall)**2-(arsall*cosiall)**2)/(1-cosiall**2)))
        trt12all = 0.5*(trt14all - trt23all)
        fit.trt14err2 = hiloerr.hiloerr(trt14all[::thinning], medtrt14)
        fit.trt12err2 = hiloerr.hiloerr(trt12all[::thinning], medtrt12)
        #Calculate stellar density in solar units (rho_s/rho_sun)
        #Using Eqn 9 from Seager & Mallen-Ornelas (2003)
        fit.rhostar2= 3*pi/(p*86400)**2/G*ars**3        #MKS Units
        medrhostar  = 3*pi/(p*86400)**2/G*arsmed**3
        rhostarall  = 3*pi/(p*86400)**2/G*arsall**3
        fit.rhostarerr2 = hiloerr.hiloerr(rhostarall[::thinning], medrhostar)
        #Calculate stellar mass in solar units (m_s/m_sun)
        #Using Eqn 10 from Seager & Mallen-Ornelas (2003)
        x = 0.8     #valid for F-K star on main sequence
        k = 1.0     #valid for F-K star on main sequence
        ...
        medmstar   = (k**3*medrhostar/rhosun)**(1/(1-3*x))
        mstarall   = (k**3*rhostarall/rhosun)**(1/(1-3*x))
        fit.mstarerr2 = hiloerr.hiloerr(mstarall[::thinning], medmstar)
        #Calculate stellar radius in solar units (r_s/r_sun)
        #Using Eqn 11 from Seager & Mallen-Ornelas (2003)
        fit.rstar2 = k*fit.mstar2**x
        medrstar   = k*medmstar**x
        rstarall   = k*mstarall**x
        fit.rstarerr2 = hiloerr.hiloerr(rstarall[::thinning], medrstar)
        #Print results
    ...
    # Check that numcalc is not too large
    if event.params.numcalc > allparams.shape[1]:
        print("WARNING! Not enough chain iterations for specified " +
              "number of temperature calculations!\nReducing numcalc " +
              "to match number of iterations.")
        event.params.numcalc = allparams.shape[1]

    # do the calc
    if hasattr(fit.i, 'depth') or hasattr(fit.i, 'depth2') or hasattr(fit.i, 'depth3') or hasattr(fit.i, 'gdepth'):
        ...
        bsdata[1] = np.random.normal(event.logg,event.loggerr,
                                     event.params.numcalc)
        bsdata[2] = np.random.normal(event.tstar,event.tstarerr,
                                     event.params.numcalc)
        event.tbntrial = bsdata[1].size

        # Step for slicing and range of values to slice
        # (slice step must be an integer, so for shape reasons
        # we must limit the range to slice ensure the right
        # size)
        slicenum = allparams.shape[1] // event.params.numcalc
        slicelim = slicenum * event.params.numcalc
        #mandelecl model
        if hasattr(fit.i, 'depth'):
            bsdata[0] = allparams[fit.i.depth,0:slicelim:slicenum]
        ...
        if hasattr(fit.i, 'depth2'):
            bsdata[0] = allparams[fit.i.depth2,0:slicelim:slicenum]
            tb, tbg, numnegf, fmfreq = calcTb(bsdata, kout, filterf, event)
            print('There were ' + str(numnegf) + ' negative flux values for which a temperature was not computed.', file=printout)
        ...
        if hasattr(fit.i, 'depth3'):
            bsdata[0] = allparams[fit.i.depth3,0:slicelim:slicenum]
            tb, tbg, numnegf, fmfreq = calcTb(bsdata, kout, filterf, event)
            print('There were ' + str(numnegf) + ' negative flux values for which a temperature was not computed.', file=printout)
        ...
        if hasattr(fit.i, 'gdepth'):
            bsdata[0] = allparams[fit.i.gdepth,0:slicelim:slicenum]
            tb, tbg, numnegf, fmfreq = calcTb(bsdata, kout, filterf, event)
            print('There were ' + str(numnegf) + ' negative flux values for which a temperature was not computed.', file=printout)
            fit.gtbm   = np.median(tb[np.where(tb > 0)])
            fit.gtbsd  = np.std(tb[np.where(tb > 0)])
            fit.gtbgm  = np.median(tbg[np.where(tbg > 0)])
            fit.gtbgsd = np.std(tbg[np.where(tbg > 0)])
            print('Band-center brightness temp = ' + str(round(fit.gtbgm,2)) + ' +/- ' + str(round(fit.gtbgsd,2)) + ' K for mandelgraz model.', file=printout)
            print('Integral    brightness temp = ' + str(round(fit.gtbm ,2)) + ' +/- ' + str(round(fit.gtbsd, 2)) + ' K for mandelgraz model.', file=printout)            
        fit.freq  = fmfreq
    ...
    if hasattr(fit.i,'gmidpt'):
        fit.gecltimeutc = (np.floor((event.bjdutc.flat[0] - ephtimeutc)/event.period) + \
                          fit.bestp[fit.i.gmidpt]) * event.period + ephtimeutc
        fit.gecltimetdb = (np.floor((event.bjdtdb.flat[0] - ephtimetdb)/event.period) + \
                          fit.bestp[fit.i.gmidpt]) * event.period + ephtimetdb
        fit.gecltimeerr = fit.typerr[fit.i.gmidpt]*event.period
        print('Grazing eclipse time = ', fit.gecltimeutc, '+/-', fit.gecltimeerr, 'BJD_UTC', file=printout)
        print('Grazing eclipse time = ', fit.gecltimetdb, '+/-', fit.gecltimeerr, 'BJD_TDB', file=printout)
        #COMPUTE MINIMUM ECCENTRICITY
        fit.geminerr    = orbit.error_ecosomega(ephtimetdb, fit.gecltimetdb, event.period,
                                     event.ephtimeerr, fit.gecltimeerr, event.perioderr)
        print('Minimum eccentricity = ', fit.geminerr[0], '+/-', fit.geminerr[1], file=printout)        
...
def calcTb(bsdata, kout, filterf, event):
    ...
    for i in range(sz):
        if bsdata[0,i] > 0:
            ...
            tb[i], tbg[i] = transplan_tb.transplan_tb(arat[i], bsdata[0,i], bsdata[1,i], bsdata[2,i], kfreq=kfreq, kgrav=kgrav, ktemp=ktemp, kinten=kinten, ffreq=ffreq, ftrans=ftrans, fmfreq=fmfreq, fstar=fstar, fmstar=fmstar)
