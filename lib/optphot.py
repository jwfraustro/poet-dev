...
def optphot(image, profile, var=None, mask=None, nochecks=False, frac=0.5):
  ...
  if not nochecks:
    ...
    if np.shape(image) != np.shape(profile):
  ...
  uncert   = np.sqrt(imvar)

  # enough good pixels?
  good = np.sum(mask) > frac * np.size(mask)

  return immean, uncert, good
