...
Revisions
---------
2018-01-07  Zacchaeus  Updated to Python 3
"""
import numpy as np
...
class tepfile:
    ...
    def __init__(self, filename, conv=True):
        ...
        valarr = np.loadtxt(filename, dtype=bytes).astype(str)
        ...
        for i, parname in enumerate(parnames):
            ...
            if self.version < 4:
                setattr(self, parname,
                        param(valarr[i, 1], valarr[i, 2], None, None))
                # exec('self.{pname} = param("{val}", "{uncert}",' \
                #      ' None, None)'.format(pname  = parname,
                #                            val    = valarr[i, 1],
                #                            uncert = valarr[i, 2]))
            elif self.version == 4:
                setattr(self, parname, param( *valarr[i, 1:5] ))
                # exec('self.{pname} = param("{val}", "{uncert}",' \
            ...
            if parname not in strvals:
                getattr(self, parname).val = \
                    np.float(getattr(self, parname).val)
                # exec("self.{pname}.val    = " \
                #      "np.float(self.{pname}.val)".format(pname = parname))
                getattr(self, parname).uncert = \
                    np.float(getattr(self, parname).uncert)
