...
def do_irsa(event, fit, topstring = '', directory = '.',
    ...
    # List of binned cols (handled differently)
    bincols = ['bestfitbin']
    ...
    for item in range(len(columns)):
        found = False
        for attr, value in event.fp.__dict__.items():
        ...
        if not found:
            for attr, value in fit.__dict__.items():
                if columns[item][0] == attr:
                    found = True
    ...
    try: # From 'event.fp.label.param' and 'event.fp.unit.param'
        for item in range(len(columns)):
            for attr, value in event.fp.label.__dict__.items():
                if columns[item][0] == attr:
                    colnames.append(value)
            for attr, value in event.fp.unit.__dict__.items():
                if columns[item][0] == attr:
                    units.append(value)
    except: # From 'columns'
        for item in range(len(columns)):
